...
```
2. 모델은 컨트롤러의 메서드에서 매개변수로 받아 온다. FirstController의 `niceToMeetYou()` 메서드에 Mdoel 타입의 `model` 매개변수를 추가한다. Model 클래스 패키지가 자동으로 임포트된다.
```java
    @GetMapping("/hi")
...
```
3. Model 패키지가 임포트됐다면 모델을 통해 변수를 등록할 수 있다. 모델에서 변수를 등록할 때는 `addAttribute()` 메서드를 사용한다.
```java
    @GetMapping("/hi")
    String niceToMeetYou(Model model) {
